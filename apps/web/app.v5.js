...
const GAME_INFO_TEXT = [
  "Привет! В этой игре не нужно ничего делать - у тебя появился персонаж, он будет играть и развиваться за тебя всё время, что будет идти концерт! Бои проходят раз в минуту, в ходе боев твой персонаж может визуально обновляться, получать новые вещи и улучшать уровень.",
  "Таблицу лидеров можешь посмотреть, нажав на кнопку [t] внизу экрана, удачи",
].join("\n");

const componentNameRuById = {
  hat_warrior: "Шлем",
  hat_warrior_starter: "Шлем",
  hat_mage: "Шляпа волшебника",
  hat_mage_starter: "Шляпа волшебника",
  hat_cowboy: "Ковбойская шляпа",
  hat_cowboy_starter: "Ковбойская шляпа",
  hat_warrior_crown: "Корона",
  hat_mage_halo: "Нимб",
  hat_cowboy_emu_kak_raz: "А она ему как раз",
  face_plain: "Обычное лицо",
  face_plain_starter: "Обычное лицо",
  face_bandana: "Лицо в бандане",
  face_bandana_starter: "Лицо в бандане",
  face_blessed_eyes: "Благословенные глаза",
  face_blessed_eyes_starter: "Благословенные глаза",
  arms_warrior: "Щит и меч",
  arms_warrior_starter: "Щит и меч",
  arms_mage: "Рубаха",
  arms_mage_starter: "Рубаха",
  arms_mage_mantle_top: "Магическое ожерелье",
  arms_mage_mantle_top_starter: "Магическое ожерелье",
  arms_cowboy: "Револьверы",
  arms_cowboy_starter: "Револьверы",
  torso_warrior: "Кираса",
  torso_warrior_starter: "Кираса",
  torso_mage: "Мантия",
  torso_mage_starter: "Мантия",
  torso_mage_mantle_bottom: "Мантия колдуна",
  torso_mage_mantle_bottom_starter: "Мантия колдуна",
  torso_cowboy: "Ремень охотника",
  torso_cowboy_starter: "Ремень охотника",
  legs_boots: "Ботинки",
  legs_boots_starter: "Ботинки",
  legs_cowboy_boots: "Ковбойские сапоги",
  legs_cowboy_boots_starter: "Ковбойские сапоги",
  legs_brass_kneepads: "Латунные наколенники",
  legs_brass_kneepads_starter: "Латунные наколенники",
};
...
function buildAsciiInfoFrame(rawText, maxInnerWidth = 44) {
  ...
  const frameLines = wrapped.map((line, index) => {
    const isParagraphBreak = line.length === 0;
    const isLastInParagraph = index === wrapped.length - 1 || wrapped[index + 1].length === 0;
    const lineText = isParagraphBreak
      ? "".padEnd(innerWidth, " ")
      : (isLastInParagraph ? line.padEnd(innerWidth, " ") : justifyLine(line, innerWidth));
    return `| ${lineText} |`;
  });
...
async function renderLiveLeaderboardPage() {
  ...
  window.setTimeout(fitAdminTableToViewport, 80);
  window.setTimeout(fitAdminTableToViewport, 260);
...
function fitAdminTableToViewport() {
  ...
  const rawScaleWidth = targetWidth / tableWidth;
  let rawScale = rawScaleWidth;

  if (document.body.classList.contains("live-leaderboard-mode")) {
    const wrapHeight = tableWrapEl.clientHeight;
    const tableHeight = adminTableEl.scrollHeight;
    if (wrapHeight > 0 && tableHeight > 0) {
      const targetHeight = Math.max(1, wrapHeight * 0.95);
      const rawScaleHeight = targetHeight / tableHeight;
      rawScale = Math.min(rawScaleWidth, rawScaleHeight);
    }
  }

  const scale = Math.max(0.2, Math.min(2.4, rawScale));
